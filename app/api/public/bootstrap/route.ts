...
import { sampleMenu, locations, createPickupTimes } from "@/lib/catalog";

export async function GET() {
  await ensureDatabase();
  const setting = await appEnv().DB.prepare("SELECT * FROM store_settings WHERE id = 'store'").first();
  const menuRows = await appEnv().DB.prepare("SELECT id,category_id,name,description,price,ingredients,allergens,spiciness,image_url,max_per_order,is_sold_out,is_recommended,is_new,is_published FROM menu_items WHERE is_published = 1 ORDER BY sort_order").all<Record<string, unknown>>();
  ...
  return Response.json({ setting, menu, locations, date, pickupTimes: createPickupTimes(), announcement: "営業日時と出店場所はInstagramでお知らせします。" }, { headers: securityHeaders() });
}
