import { z } from "zod";
import { appEnv, ensureDatabase, securityHeaders, sha256 } from "@/lib/server";
...
export async function POST(request: Request) {
  ...
  if (!parsed.success) return Response.json({ error: "注文番号と6桁の確認コードを入力してください" }, { status: 400 });
  ...
  if (!row) return Response.json({ error: "該当する注文を確認できません" }, { status: 404, headers: securityHeaders() });
  return Response.json({ order: row }, { headers: securityHeaders() });
}
