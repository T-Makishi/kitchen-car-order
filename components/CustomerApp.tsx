...
export function CustomerApp() {
  ...
  const isOpen = setting.order_status === "OPEN";
  ...
  return <>
    ...
    <main id="main">
      <section className="hero" id="top"><div className="shell hero-grid"><div className="hero-copy-column"><p className="eyebrow">PRE-ORDER / PICK UP</p><h1>できたてを、<br/><em>待たずに。</em></h1><p className="hero-copy">受取時間を選んで、店頭で受け取る。<br/>お支払いは受取時にSquareで。</p><div className="hero-actions"><a className="button" href="#menu">メニューを見る <span>↘</span></a><button className="button secondary" onClick={() => setDialog({ type: "status" })}>注文状況を確認</button></div><div className="status-card"><span className={`status-dot ${isOpen ? "" : "closed"}`}/><span><strong>{isOpen ? "事前注文を受付中" : "ただいま受付時間外です"}</strong><small>{todayLocation ? `本日の販売場所：${todayLocation.name}` : "本日の販売場所は準備中です"}</small></span>{todayLocation && <a href="#shop">地図を見る ↓</a>}</div></div><figure className="hero-card"><img src="/og.png" alt="ベージュ色のキッチンカーを描いたダーク・ストリート調のイラスト"/><figcaption><span>NIGHT SHIFT</span><strong>ORDER / PICK UP<br/>ENJOY</strong></figcaption></figure></div></section>
